...
#         N, C, H, W = x.shape
#         # used for backup if there are less number of regions
#         bb_x = [
...
#                     if (
#                         idx == rr_boxes[b_idx]
#                         and obj_i[b_idx] > 10000
#                         and len(img_nodes) < self.NB - 2
#                     ):
...
#                     x_cropped = x[Nx][
#                         :, bb_x[i][1] : bb_x[i][3], bb_x[i][0] : bb_x[i][2]
#                     ]
...
def evaluateModel(args, model, up, image_list):
    ...
    for fname in image_list:
        ...
        x.append(img)
    batch = torch.stack(x)
    img_out = model(img.unsqueeze(0).cuda())  #36,20,480,640
    
    output = img_out.squeeze(0)  # Remove batch dimension
    ...
    # Save the mask image
    cv2.imwrite('path_to_save_mask.png', color_mask)
    ...
    for jj in range(len(mask)):  #batch processing
        ...
        for i in range(len(obj_ids)):
            label_img = to_image(single_label_mask == obj_ids[i])
            label_img.save(args.savedir + os.sep + 'label_'+str(i)+'_' + name.replace(args.img_extn, 'png'))
        
        masks = single_label_mask == obj_ids[:, None, None]
        
        # Save semantic images using labels
        for i in range(len(img_stk)):
            label_img = PILImage.fromarray(classMap_numpy_color)
            label_img.save(args.savedir + os.sep + 'label_'+str(i)+'_' + name.replace(args.img_extn, 'png'))
        ...
        # Sort the boxes                
        # rr =./home/leo/usman_ws/datasets/examples-espnet/data/*.jpg ((bb_x[:, 2])-(bb_x[:, 0]))*((bb_x[:, 3])-(bb_x[:, 1]))
